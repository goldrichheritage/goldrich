...
fetch("collection-data.json")
    ...
    .then(function(data) {
        ...
        data.forEach(function(object) {
            ...
            if (object.collectionNumber !== "FDA001") {
                card.classList.add("compact-image-card");
            }
            if (object.collectionNumber === "FDA011") {
                card.classList.add("tall-image-card");
            }
            const chineseName = object.chineseName
                ? `<h3>${object.chineseName}</h3>`
                : "";
            const period = object.period
                ? `<p>Period: ${object.period}</p>`
                : "";
            card.innerHTML = `
                <a class="collection-card-image" href="${object.detailPage}" aria-label="View ${object.name}">
                    <img src="${object.image}" alt="${object.chineseName || object.name}">
                </a>
                <div class="collection-card-content">
                    <h2><a href="${object.detailPage}">${object.name}</a></h2>
                    ${chineseName}
                    <p>Collection Number: ${object.collectionNumber}</p>
                    ${period}
